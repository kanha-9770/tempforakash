...
interface MachineData {
  ...
  specification_image: Array<{ first?: string; second?: string; third?: string; fourth?: string; fifth?: string }>;
  status: string;
  rating: {
    stars: number;
    reviews: number;
    ...
    <div className="h-full bg-white rounded-xl p-4 gap-4">
      <div className="h-[75vh] p-4 rounded-xl flex flex-col">
        ...
        <div className="w-full h-[60%] justify-start">
          ...
          <span className="text-red-500 ml-2 font-bold text-3xl">Description</span>
          <div className="text-sm text-black mt-6 font-poppins">
            {machine.product_description}
          </div>
        </div>
        <div className="relative items-center h-[40%] flex flex-row justify-center">
          ...
          <div className="h-48 w-64 flex flex-col border-l-2 border-gray-400 border-r-2">
            <LottieAnimation className="h-20 -rotate-45 w-20" animationData={size} />
