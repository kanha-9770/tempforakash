...
const ProductLayout: React.FC<ProductLayoutProps> = ({ setHoveredItem, setHeading, setIsVisible }) => {
  const productData = data.find((item) => item.category === "Product")?.data
  const navLeftData = productData?.Machines || []
  const navRightData = productData?.SidebarLinks || []

  const [hoveredCategory, setHoveredCategory] = useState<string>(navRightData[0]?.name || "")
  const [currentIndex, setCurrentIndex] = useState(0)
  const [sidebarIndex, setSidebarIndex] = useState(0)
  const [expandedItem, setExpandedItem] = useState<string | null>(null)
  ...
  const handleSidebarNext = useCallback(() => {
    setSidebarIndex((prevIndex) => Math.min(prevIndex + 1, navRightData.length - 8))
  }, [navRightData.length])
  ...
  const handleCategoryClick = useCallback((linkName: string, heading: string) => {
    setHoveredCategory(linkName)
    setHoveredItem(linkName)
    setHeading(heading)
    setIsVisible(false)
  ...
  const renderMachineItem = useCallback((machine: any, index: number) => (
    <motion.div
      key={`${machine.name}-${index}`}
      className="text-center relative w-1/3 p-2"
      custom={index}
  ...
  const renderSidebarItem = useCallback((link: any, index: number) => (
    <motion.div
      key={link.name}
      custom={index}
      initial="hidden"
    ...
    >
      <a className="flex w-full gap-2 flex-row" href={`${countryCode}/products/${link.name}`}>
        <div className="flex items-center justify-center cursor-pointer">
          <BlurImage
            className="rounded-full h-6 w-6 transform transition-transform duration-200 object-cover"
            src={link.icon}
  ...
  return (
    <div ref={containerRef} className="w-full lg:w-screen z-30 h-full flex max-w-screen-2xl mx-auto items-start justify-center font-light">
      {/* Desktop View */}
      <div className="w-full hidden lg:flex flex-col lg:flex-row rounded-lg overflow-hidden">
        <div className="flex justify-center items-center w-full md:w-3/4 relative">
          {filteredMachines.length > totalVisible && (
          ...
          <div className="flex flex-wrap justify-start items-start overflow-hidden w-full">
            {filteredMachines.length <= totalVisible
              ...
              : filteredMachines.slice(currentIndex, currentIndex + totalVisible).map(renderMachineItem)}
          </div>
          ...
          {filteredMachines.length > totalVisible && (
            <button
              onClick={handleNext}
              className={`absolute text-black border-2 text-3xl rounded-full right-2 z-10 h-10 w-10 animated-button-right ${
                currentIndex + totalVisible >= filteredMachines.length ? "opacity-20" : "opacity-100"
              }`}
              style={{ top: "50%", transform: "translateY(-50%)" }}
              disabled={currentIndex + totalVisible >= filteredMachines.length}
            >
        ...
        <div className="w-full lg:w-1/4 lg:h-[26rem] flex flex-col">
          <div className="w-full h-full flex justify-center items-center border-l overflow-y-hidden border-gray-300 relative">
            ...
            <div className="space-y-4 pt-6 h-[24rem]">
              {navRightData.slice(sidebarIndex, sidebarIndex + 8).map(renderSidebarItem)}
            </div>
            {sidebarIndex + 8 < navRightData.length && (
              <button
                onClick={handleSidebarNext}
                className="absolute bottom-4 left-1/2 text-4xl text-black"
              >
                <MdKeyboardArrowDown />
              </button>
            )}
      ...
      <div className="absolute w-full h-screen p-2 -mt-2 flex lg:hidden flex-col rounded-lg overflow-hidden">
        <div className="flex flex-col h-full w-full relative">
          <div className="absolute w-full h-full flex flex-col">
            <div className="w-full flex justify-start items-start overflow-y-hidden relative">
              <div className="space-y-4 pb-32 h-full stopscrollProduct overflow-y-auto w-full">
                {navRightData.slice(sidebarIndex, navRightData.length).map((link, index) => (
                  <motion.div
                    key={index}
                    initial={{ height: expandedItem === link.name ? "auto" : "3rem" }}
                    animate={{ height: expandedItem === link.name ? "auto" : "3rem" }}
                    transition={{ duration: 0 }}
                    onClick={() => handleCategoryClick(link.name, link.name)}
                    className="flex flex-col border-b-[1px] justify-between text-lg transition-colors duration-300 cursor-pointer font-poppins text-[#483d78] font-semimedium overflow-hidden"
                  >
                    <div onClick={() => expandItem(link.name)} className="flex items-center justify-between">
                      <div className="flex flex-row space-x-3 cursor-pointer">
                        <div className="flex items-center justify-center">
                          <BlurImage
                            className="rounded-full h-6 w-6 ml-1 duration-200 object-cover"
                        ...
                        <p className={expandedItem === link.name ? "text-[#483d73]" : "text-gray-500"}>
                          {link.name}
                        </p>
                      </div>
                                ...
                                ? filteredMachines.map((machine, index) => (
                                    ...
                                    >
                                      <Image
                                        ...
                                        height={150}
                                      />
                                      <h3 className="text-sm text-black mt-2 w-full font-bold">{machine.name}</h3>
                                    </motion.div>
                                  ))
                                : filteredMachines
                                    .slice(currentIndex, currentIndex + mobileVisibleItems)
                                    .map((machine, index) => (
                                      <motion.div
                                        key={`${machine.name}-${index}`}
                                        className="text-center h-40 rounded-xl p-2 border-2"
                                      ...
                                      >
                                        <BlurImage
                                          ...
                                          loading="lazy"
                                        />
                                        <h1 className="text-sm text-black font-bold mt-2">{machine.name}</h1>
                                      </motion.div>
                                    ))}
                            </div>
                            <div className="relative w-full space-x-4 flex z-30 h-[5%] justify-center items-center">
                              {filteredMachines.length > mobileVisibleItems && (
                                <>
                                  ...
                                  <button
                                    onClick={handleNext}
                                    className={`text-black z-30 text-3xl transition-all ${
                                      currentIndex + mobileVisibleItems >= filteredMachines.length
                                        ? "opacity-20"
                                        : "opacity-100"
                                    }`}
                                    disabled={currentIndex + mobileVisibleItems >= filteredMachines.length}
                                  >
                                    <FaArrowRightLong />
                                  </button>
                                </>
